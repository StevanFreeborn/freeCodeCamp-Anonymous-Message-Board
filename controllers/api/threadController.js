...
export default class ThreadController {
    ...
    static deleteThreadById = async (req, res) => {
        ...
        const isCorrectPassword = await HashService.compare(delete_password, thread.delete_password);
        ...
        if (threadDeletion.acknowledged == false) {
            return res.status(400).json({ error: `Unable to delete thread with id ${thread.id}`, });
        }

        
        const repliesDeletion = await ReplyService.deleteRepliesByThreadId(thread.id);

        if (repliesDeletion.acknowledged == false) {
            return res.status(400).json({ error: `Unable to delete the replies for thread with id ${thread.id}`, });
        }
